...
# ── Role aliases  ─────────────────────────────────────────────────────────────
# Each key is a canonical label; values are all phrases that should match it.
...
def rank_candidates(
    ...
    top_n: int = TOP_N,
    min_experience: float | None = None,   # e.g. 2.0  → only ≥ 2 yrs
) -> list[dict]:
    """
    ...
    # Resolve canonical role from query once
    canonical_query_role = _canonical_role(query_clean)

    # Parse min_experience from the query string if not passed explicitly
    ...
    for c in candidates:
        ...
        if min_experience is not None:
            ...
            if exp_yrs is not None and exp_yrs < min_experience:
                continue           # skip under-experienced candidate
            # If experience_years field is missing entirely, don't hard-exclude
            # (the extractor may not have found it); let score decide.
        ...
        # Hard penalty: if role clearly doesn't match, cap at 15 %
    ...
    # After sorting, only return candidates whose score is meaningful
    # (don't surface 4 % matches just because they're the only ones)
...
def _canonical_role(query: str) -> str | None:
    """Map query text to a canonical role key, or None if unrecognised."""
    q = query.lower()
    for canon, aliases in ROLE_ALIASES.items():
        for alias in aliases:
            if alias in q:
                return canon
    # Partial: check if any single word in the alias appears prominently
    for canon, aliases in ROLE_ALIASES.items():
...
def _role_matches_canonical(candidate_role: str, canonical: str) -> bool:
    """True if candidate_role belongs to the same canonical group."""
...
def _role_match(
    ...
    """
    Returns (score 0-1, role_matched: bool).
    score    — how well roles align
    role_matched — True if we're confident roles are the same category
    """
    ...
    # 1. Canonical alias match (highest confidence)
    if canonical_query_role:
        if _role_matches_canonical(cand_lower, canonical_query_role):
            return 1.0, True
        else:
            # Check reverse: maybe candidate's role maps to a DIFFERENT canonical
            cand_canonical = _canonical_role(cand_lower)
            if cand_canonical and cand_canonical != canonical_query_role:
                return 0.0, False   # definitively different roles
            # Unknown role — partial word overlap
            return _word_overlap_score(query, cand_lower), False

    # 2. No canonical recognised — fall back to word overlap
...
def _parse_experience_years(candidate: dict) -> float | None:
    ...
    try:
        ...
        pass
    # Fallback: parse from experience text
    exp_text = candidate.get("experience", "") or ""
...
def _parse_min_experience_from_query(query: str) -> float | None:
    """
    Detect phrases like 'at least 2 years', 'minimum 3 yrs', '5+ years'.
    Returns the number as float, or None.
    """
    patterns = [
